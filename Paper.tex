...
\title[Ethereum: A Secure Decentralised Generalised Transaction Ledger\\ \smaller \textbf{{London version}}]{Ethereum: A Secure Decentralised Generalised Transaction Ledger \\ \smaller \textbf{{London version \YellowPaperVersionNumber}}}
\author{
    Dr. Gavin Wood\\
...
This document describes one version of the protocol, namely the \textit{London} version defined by \cite{BeikoLondon}.
...
EIP-2718 by \cite{EIP-2718} introduced the notion of different transaction types, formally $T_{\mathrm{x}}$.
As of the London version of the protocol, there are tree transaction types: 0 (legacy), 1 (EIP-2930 by \cite{EIP-2930}) and 2 (EIP-1559 by \cite{EIP-1559}).

Legacy transactions can be encoded either as
\begin{eqnarray}
&\textbf{Payload}& \\ 
&\nonumber \text{or} \\
&0 \cdot \textbf{Payload}&
\end{eqnarray}
and is the only that has $T_x$ optional

It is formed by the fields:
\begin{description}
...
\item[data] An unlimited size byte array specifying the input data of the message call, formally $\mathbf{p}$.
...
\item[yParity]\linkdest{tx_y_parity}{} Signature Y parity; formally $T_{\mathrm{y}}$, this is included in $T_w$ of legacy transactions but here is also represented seperated.
\end{description}

EIP-1559 (type 2) removes $gasPrice$ and replaces it with:
\begin{description}
\item[maxPriorityFeePerGas]\linkdest{tx_max_priority_fee_per_gas}{} TODO
\item[maxFeePerGas]\linkdest{tx_max_fee_per_gas}{} TODO
\end{description}

Further, there are two subtypes of transactions: those with a defined $T_t$ which result in message calls and those which result in the creation of new accounts with associated code (known informally as `contract creation'). 

For the first type it is defined:
\begin{description}
...
For the second:
\begin{description}
\item[init] The code that sets up the a smart contract, formally $T_{\mathbf{i}}$.
\end{description}
...
(T_{\mathrm{c}}, T_{\mathrm{n}}, T_{\mathrm{p}}, T_{\mathrm{g}}, T_{\mathrm{t}}, T_{\mathrm{v}}, \mathbf{p}, T_{\mathbf{A}}, T_{\mathrm{y}}, T_{\mathrm{r}}, T_{\mathrm{s}}) & \text{if} \; T_{\mathrm{x}} = 2 TODO
\end{cases}
\end{equation}
...
If the calculated address has nonce different than zero and/or already has a code associated with it the execution halts here (see EIP-684 by \cite{EIP-684}).
